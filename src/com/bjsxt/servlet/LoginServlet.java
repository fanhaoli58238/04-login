...
*               //响应处理结果
*   请求转发学习:
*               作用:实现多个servlet联动操作处理请求,这样避免代码冗余,让servlet职责更加明确。
*               使用:
...
*   重定向:
*                   解决了表单重复提交的问题,以及当前servlet无法处理请求的问题。
*                   使用:
 ...
 *   解决主页面用户名显示为null的问题:
 *                  原因:
 *                          因为在用户登录成功后使用重定向显示主页面,两次请求,而用户的信息在第一次请求中,
 *                          第二次请求中没有用户数据,所以显示为null
 *                  解决:
 *                          使用session
 *   使用ServletContext对象完成网页计数器
 *                  在用户登录校验中创建用户计数器并自增,然后存储到ServletContext对象中
 *                  在主页面里取出计数器显示给用户即可。
* */
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        //响应处理结果
                if(u!=null){
                    ...
                    //使用request对象实现不同Servelt不同数据流转
                    req.setAttribute("str","用户名和密码错误");
                    //使用请求转发
